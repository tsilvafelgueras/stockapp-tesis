...
export default async function ConfirmarLayout({
  children,
}: {
  ...
  if (profile?.role !== 'operario' && !isSuperActuando) {
    redirect(
      `${homePathForRole(profile?.role, profile?.empresa_id_actuando)}?denegado=confirmar`
    )
  }
  ...
  return (
    <AppShell
      role={profile!.role as 'operario' | 'super'}
